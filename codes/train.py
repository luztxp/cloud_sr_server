...
testset = agora15_val.DatasetFromFolderVal("../datasets/test_data/hr_img/",
                                       "../datasets/test_data/lr_img/x{}/".format(args.scale),
                                           args.scale)
training_data_loader = DataLoader(dataset=trainset, num_workers=0, batch_size=args.batch_size, shuffle=True, pin_memory=True, drop_last=True)
testing_data_loader = DataLoader(dataset=testset, num_workers=0, batch_size=args.testBatchSize,
                                 shuffle=False)

print(f"********** len of trainset{len(training_data_loader)}")
print(f"********** len of trainset{len(testing_data_loader)}")
...
def valid():
    ...
    for i, batch in enumerate(testing_data_loader):
        ...
        avg_psnr += psnr
        avg_ssim += ssim
        print(f" Valid {i}/{len(testing_data_loader)} with PSNR = {psnr} and SSIM = {ssim}")
